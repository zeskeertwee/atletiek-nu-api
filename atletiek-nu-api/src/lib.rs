...
pub use reqwest::{blocking::Request, StatusCode};
...
pub fn send_request(url: &str) -> anyhow::Result<String> {
...
pub fn get_competitions() {
    let url = "https://www.atletiek.nu/athleteapp.php?page=events&do=get&country_iso2=NL&version=1.16&language=en_GB&improvePerformance=0";
    let body = send_request(url).unwrap();
    let html = Html::parse_fragment(&body);
    models::competitions_list::parse(html).unwrap();
}

#[deprecated]
pub fn search_competitions(q: &str) -> anyhow::Result<CompetitionsList> {
    ...
    let body = send_request(&url)?;
    models::competitions_list::parse(Html::parse_fragment(&body))
}

pub fn search_athletes(q: &str) -> anyhow::Result<AthleteList> {
...
pub fn get_competition_registrations<C: CompetitionID>(
    competition_id: &C,
...
pub fn get_athlete_event_result(participant_id: u32) -> anyhow::Result<AthleteEventResults> {
...
pub fn get_competitions_for_time_period(
    start: NaiveDate,
    end: NaiveDate,
) -> anyhow::Result<CompetitionsWebList> {
    search_competitions_for_time_period(start, end, "")
}

pub fn search_competitions_for_time_period(
    start: NaiveDate,
    end: NaiveDate,
    q: &str,
) -> anyhow::Result<CompetitionsWebList> {
